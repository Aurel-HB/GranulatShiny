...
Dans cette partie on s'intéresse aux indicateurs de biodiversité et d'abondance. Les indicateurs présentés sont ceux référencés dans le "Protocole halieutique" article 8.4.1.
...
où : **S** est le nombre total d'espèces, <br> **pi** est la proportion de la i-ème espèce parmi toutes les espèces présentes, <br> **ln** représente le logarithme népérien
...
Dans un premier temps, le tableau (ci-dessous) affiche les valeurs moyennes d’abondance, de biomasse, de richesse spécifique, des indicateurs de Shannon et de Simpson à l’intérieur de la concession, à l’extérieur de la concession, et au global pour chaque campagne. Ils sont calculées à partir des valeurs obtenues en chaque station échantillonnée. Pour plus de lisibilité, les écarts-types ne sont pas affichés dans le tableau présent dans l'application mais sont disponibles dans le fichier cvs téléchargeables via le bouton "Telecharger le tableau".
...
Cet onglet permet de choisir et de visualiser la distribution de probabilité qui correspond le mieux à la variable expliquée. L'histogramme de fréquence (en gris dans l'exemple) représente la distribution empirique des données observées. Il est construit en regroupant les données en intervalles et en comptant le nombre d'observations dans chaque intervalle. Cela donne une visualisation de la répartition des valeurs de la variable.<br> La fonction de densité (en bleu dans l'exemple) est une estimation de la distribution de probabilité des données. Elle est calculée en ajustant différents modèles de distribution statistique aux données observées. Ces modèles peuvent inclure des distributions normales, de Poisson, exponentielles, etc. La fonction de densité représente la probabilité qu'une observation tombe dans une plage particulière de valeurs.<br> La distribution de probabilité (en vert dans l'exemple) représente le modèle de distribution statistique qui correspond le mieux aux données observées. C'est le modèle qui a été ajusté aux données et qui est estimé être le plus approprié pour représenter la distribution sous-jacente des données. Les paramètres de chaque distribution de probabilité sont approximés à l'aide de la moyenne et de l’écart type de la variable.
...
La Permanova est une technique d'analyse non paramétrique, offrant une alternative à l'analyse de variance classique. Contrairement à cette dernière, la Permanova ne repose pas sur l'hypothèse de distribution linéaire des données ou sur une homogénéité des variances. Sa robustesse face à ces contraintes en fait un outil plus flexible. En effet, elle peut être appliquée à des données de toutes dimensions, qu'elles soient univariées ou multivariées, en fonction de un ou deux facteurs qualitatifs.

L'hypothèse nulle de la Permanova stipule qu'il n'existe pas de différence significative dans la position et/ou la dispersion des groupes définis par les modalités des facteurs qualitatifs dans un espace multidimensionnel. Pour évaluer cette hypothèse, la technique construit une matrice d'association entre les éléments, répertoriant toutes les distances entre chaque paire d'éléments. Ensuite, elle calcule la somme des carrés des distances inter-groupe et intra-groupe. Cette approche est analogue à une analyse de variance classique, où la variance inter-groupe est comparée à la variance intra-groupe.

La décision de rejeter ou non l'hypothèse nulle se fait en examinant le rapport entre la variance inter-groupe et la variance intra-groupe. Si ce rapport est significativement grand, cela suggère que les différences observées entre les groupes sont probablement réelles. Pour établir la significativité, la Permanova utilise une distribution obtenue par permutation des données. Cela implique de redistribuer aléatoirement les données un grand nombre de fois pour obtenir la distribution sous l'hypothèse nulle.

Cependant, un inconvénient de la Permanova est qu'elle ne permet pas de déterminer quel groupe spécifique est différent des autres, seulement qu'au moins un groupe diffère. De plus, lorsque deux éléments ont des valeurs nulles, cela peut biaiser l'estimation de la similarité entre eux, ce qui est problématique en écologie où un zéro peut signifier l'absence d'une espèce. Pour résoudre ce problème, le choix du coefficient d'association dans le calcul de la matrice de distance peut prendre en compte ou non ces valeurs nulles, en fonction de l'objectif de l'analyse et des données disponibles.
...
Pour un GLMM et un GLM, vous devrez choisir une distribution de probabilité. Par défaut, il propose la dernière distribution de probabilité que vous avez vérifiée dans la partie précédente. Attention la méthode utilisée pour la modélisation est itérative, il se peut donc que la distrubution qui semblait la plus adéquate dans la partie précédente n'est pas forcément celle qui permettra de mieux faire converger le modèle. Néanmoins l'onglet "diagnostic d'analyse" devrait avoir permis de sélectionner un nombre de distribution possible pour ne pas avoir à toutes les tester ici.<br>
...
La parite haute de la fenêtre de résultat rappelle le modèle qui a été utilisé pour calculer les effets.
...
Ensuite, il est possible de lire des scores liées à la vraisemblance du modèle par rapport aux données et aux paramètres sélectionnés pour la construction du modèle.

Etant donné un échantillon observé $$ (x_1,\ldots,x_n)$$ et une loi de probabilité $$ P_\theta$$, la vraisemblance quantifie la probabilité que les observations proviennent effectivement d'un échantillon (théorique) de la loi $$ P_\theta$$. On appelle vraisemblance associé à la loi de probabilité $$ P_\theta$$, la fonction **L** tel que : $$\displaystyle L(x_1,\ldots,x_n,\theta) = \prod_{i=1}^n P_\theta(x_i)\; $$
