...
La PERMANOVA est une technique d'analyse non paramétrique, offrant une alternative à l'analyse de variance classique et aux modèles linéaires (et/ou généralisés et/ou mixtes). Contrairement à ces derniers, la PERMANOVA ne repose pas sur l'hypothèse de distribution linéaire des données selon une loi de distribution de probabilité. La PERMANOVA ne fait pas d'hypothèse de distribution mais suppose que les données peuvent être représentées sous la forme d'une matrice de distance. Cela lui permet d'être appliquée à des données de toutes dimensions, qu'elles soient univariées ou multivariées, en fonction de un ou deux facteurs qualitatifs peu importe le nombre de modalité. Néanmoins l'homogénéité des variances reste nécessaire comme pour l'analyse de variance classique.

La PERMANOVA regarde s'il existe une différence significative dans la variabilité des groupes définis par les modalités des facteurs qualitatifs. Pour évaluer cela, la technique construit une matrice d'association entre les éléments, répertoriant toutes les distances entre chaque paire d'éléments. Ensuite, elle calcule la somme des carrés des distances inter-groupe et intra-groupe.
La somme des carrés des distances inter-groupe (SS inter) est une mesure de la variation entre les différents groupes ou traitements dans vos données. Elle indique à quel point les moyennes des groupes sont différentes les unes des autres. Plus la SS inter est élevée, plus les moyennes des groupes sont différentes, ce qui suggère des effets significatifs des variables indépendantes sur la variable dépendante. La somme des carrés des distances intra-groupe (SS intra) est une mesure de la variation à l'intérieur des groupes ou traitements. Elle indique à quel point les observations à l'intérieur de chaque groupe sont similaires entre elles. Si la SS intra est faible, cela signifie que les observations à l'intérieur de chaque groupe sont assez homogènes. Cette approche est analogue à une analyse de variance classique, où la variance inter-groupe est comparée à la variance intra-groupe.
...
Cependant, un inconvénient de la PERMANOVA est qu'elle ne permet pas de déterminer quel groupe spécifique est différent des autres, seulement qu'au moins un groupe diffère. De plus, lorsque deux éléments ont des valeurs nulles, cela peut biaiser l'estimation de la similarité entre eux, ce qui est problématique en écologie où un zéro peut signifier l'absence d'une espèce. Pour résoudre ce problème, en fonction du coefficient d'association choisi dans le calcul de la matrice de distance, celui-ci peut prendre en compte ou non ces valeurs nulles, en fonction de l'objectif de l'analyse et des données disponibles.
