...
indice_computing <- function(data_abun, data_biom) {
  ...
  data_abun$Abun <-
    round(rowSums(data_abun[, -c(1:9)], na.rm = T), digits = 3) # Somme des abondances par ligne = abondance totale par trait
  ...
  data_abun$Shannon <-
    round(diversity(data_abun[, -c(1:12)], index = "shannon"), digits = 3)
  data_abun <- data_abun %>% relocate(1:12, Shannon)
  data_abun$Simpson <-
    round(diversity(data_abun[, -c(1:13)], index = "simpson"), digits = 3)
  data_abun <- data_abun %>% relocate(1:13, Simpson)
  data_abun$Pielou <-
    round(data_abun$Shannon / log(data_abun$Richness), digits = 3)
